...
After intercepting some of their messages, which looked like complete nonsense, the agency learned that they indeed encrypt their messages, and studied their method 
of encryption.
Their messages consist of lowercase latin letters only, and every word is encrypted separately as follows:
Convert every letter to its ASCII value. Add 1 to the first letter, and then for every letter from the second one to the last one, add the value of the previous letter. 
...
The FBI needs an efficient method to decrypt messages. Write a function named decrypt(word) that receives a string that consists of small latin letters only,
...
public static string Decrypt(string word)
{
      ...
      if(word.Equals(word.ToLower()) == true)
      {
        for(int i=0; i< word.Length; i++)        
        {
          ...
          {                                           
            ...
            decryptedWord.Append((char)(charValue - 1));    
          }       
          else
          {            
            charValue = charValue - prevValue;
            while(charValue < 97)
            {
              charValue = charValue + 26;
            }
            decryptedWord.Append((char)(charValue));           
          }          
           prevValue = (int)(word[i]);
        }
      }  
      else
      {
